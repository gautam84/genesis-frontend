...
export default function WsdSensesAdminPage() {
  ...
  const reload = async () => {
    ...
    setError(null);
    try {
      const res = await wsdApi.listSenses(workspaceId);
      setSenses(res.data);
    } catch (err) {
      console.error(err);
      setError('Failed to load senses. You may not have access to this workspace.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    setError(null);
    try {
      await wsdApi.createSense(workspaceId, {
        word: newWord.trim(),
      ...
      console.error(err);
      setError('Failed to create sense. Admin role is required.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSaveEdit = async (senseId: string) => {
    setError(null);
    try {
      await wsdApi.updateSense(workspaceId, senseId, {
        word: editWord.trim(),
      ...
      console.error(err);
      setError('Failed to update sense.');
    }
  };
  ...
  const handleDelete = async (senseId: string) => {
    ...
    setError(null);
    try {
      await wsdApi.deleteSense(workspaceId, senseId);
      await reload();
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      setError(message || 'Failed to delete sense.');
      console.error(err);
    }
  };
